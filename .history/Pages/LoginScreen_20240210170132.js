  ...
  const handleLogin = async () => {
    try {
      ...
      const response = await fetch('http://your-server-url/login', {
        ...
        body: JSON.stringify({ email: 'user@example.com', password: 'password' }), // Utiliser l'adresse e-mail au lieu du nom d'utilisateur
      });
      ...
      // Redirection vers l'écran d'accueil ou une autre page appropriée après la connexion réussie
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        value={email}
        onChangeText={text => {
            // Vérifier si l'adresse e-mail est valide à chaque modification du texte
            const isValidEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(text);
            // Mettre à jour l'état de l'e-mail uniquement si l'adresse e-mail est valide
